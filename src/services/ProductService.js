...
export const getAllProduct = async (search) => {
    let res = {}
    if (search?.length > 0) {
        res = await axios.get(`${process.env.REACT_APP_API_KEY}/product/getAll?filter=name&filter=${search}`)
    } else {
        res = await axios.get(`${process.env.REACT_APP_API_KEY}/product/getAll`)
    }
    return res.data
}
